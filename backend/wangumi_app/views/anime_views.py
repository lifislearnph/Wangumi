...
class AnimeListCreateView(APIView):
    ...
    def post(self, request):
        ...
        cover_url_value = (
            (data.get('cover_url') or data.get('coverUrl') or data.get('cover') or "")
        )
        ...
        # 如果上传了封面文件，但 cover_url 为空，则补上文件的可访问 URL
        if cover and (not anime.cover_url):
            try:
                anime.cover_url = anime.cover_image.url
                anime.save(update_fields=["cover_url"])
            except Exception:
                # 如果无法解析 URL，忽略，避免影响主流程
                pass
...
def anime_modify(request):
    ...
    if request.method not in ['POST', 'PATCH']:
        return JsonResponse({
            "code": 405,
            "message": f"Method {request.method} not allowed. Use POST or PATCH.",
            "data": None
        }, status=405)

    content_type = request.content_type or ""

    # 对于 multipart/form-data，使用 request.POST 和 request.FILES
    if "multipart/form-data" in content_type:
        ...
        try:
            ...
            return build_error_response("JSON格式错误", status=400)
    else:
        # 对于其他格式，尝试使用 request.POST
        data = request.POST
    ...
    # 检查是否有任何修改内容
    has_modifications = any([title, description, cover_url, cover])
    ...
    # 如果上传了封面文件但 cover_url 仍为空，补上文件 URL（方便前端依赖 cover_url 的场景）
    if cover and (not anime.cover_url):
        try:
            anime.cover_url = anime.cover_image.url
            anime.save(update_fields=["cover_url"])
