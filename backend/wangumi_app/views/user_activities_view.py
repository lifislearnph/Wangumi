...
def get_watch_status_text(status):
    """将 WatchStatus 状态码转换为可读文本"""
    status_map = {
        'WANT': '想看',
        'WATCHING': '在看', 
        'FINISHED': '已看'
    }
    return status_map.get(status, '追番')
    ...
    def get(self,request):
        ...
        for act in page_obj:
            ...
            if act.action in ["创建了评论", "COMMENT"]:
                ...
                if target:
                    ...
                    if target.scope == 'ANIME':
                        ...
                        if anime:
                            title = anime.title
                            anime_item_relatedID = target.object_id
                        ...
                        if episode and episode.anime:
                            ...
                            anime_item_relatedID = episode.anime.id
                        ...
                        if item:
                            title = item.title
                            anime_item_relatedID = target.object_id
            elif act.action in ["WATCHING"]:
                target = watch_map.get(act.object_id)
                if target and target.anime:
                    title = target.anime.title
                    extra_data = "在看"
                    anime_item_relatedID = target.anime.id
                else:
                    continue  # 跳过无效记录
            elif act.action in ["WANT"]:
                target = watch_map.get(act.object_id)
                if target and target.anime:
                    title = target.anime.title
                    extra_data = "想看"
                ...
                if target:
                    title = target.title
                    anime_item_relatedID = target.id
                else:
                    continue  # 跳过无效记录
            ...
            results.append({
                ...
                "target_id": anime_item_relatedID,
                "target_type": act.content_type.model,
                "target_title": title,
                "extra_data": extra_data,  # 包含状态信息
            })
