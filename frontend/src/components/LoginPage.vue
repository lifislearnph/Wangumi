  ...
  try {
    ...
    if (data.access && data.refresh) {
      ...
      try {
        ...
        if (userResponse.data.code === 0 && userResponse.data.data?.avatar) {
          const avatar = userResponse.data.data.avatar;
          localStorage.setItem('user_avatar', avatar);
          console.log('头像加载成功:', avatar);

          // 触发头像更新事件
          window.dispatchEvent(new CustomEvent('avatar-updated', { detail: { avatar } }));
        } else {
          console.log('用户没有头像或接口返回格式异常');
        }
      } catch (userError) {
