  ...
  <div class="episode-detail-page">
    ...
    <main class="detail-content" v-else-if="episodeDetail">
      ...
      <div class="episode-details">
        ...
        <!-- 在线观看链接 -->
        <div class="episode-online-links" v-if="episodeDetail?.online_urls?.length > 0">
          <h3>
          ...
          <div class="links-grid">
            <a
              v-for="(link, index) in episodeDetail.online_urls"
              :key="index"
              :href="link.url"
              target="_blank"
              class="link-card"
            >
              ...
              <div class="link-info">
                <div class="link-site">{{ link.site || `观看平台 ${index + 1}` }}</div>
        ...
        <!-- 原始数据展示（用于调试） -->
        <div class="episode-raw-data" v-if="episodeDetail">
          <h3>
          ...
          <div class="data-grid">
            <div class="data-item">
              <span class="data-label">集数:</span>
              <span class="data-value">{{ episodeDetail.id }}</span>
            </div>
            <div class="data-item">
              <span class="data-label">英文标题:</span>
              <span class="data-value">{{ episodeDetail.title }}</span>
            </div>
            <div class="data-item">
              <span class="data-label">中文标题:</span>
              <span class="data-value">{{ episodeDetail.subtitle }}</span>
            </div>
            <div class="data-item">
              <span class="data-label">播出日期:</span>
              <span class="data-value">{{ formatDate(episodeDetail.air_date) }}</span>
            </div>
            <div class="data-item">
              <span class="data-label">描述:</span>
              <span class="data-value">{{ episodeDetail.description || '暂无描述' }}</span>
            </div>
            <div class="data-item">
              <span class="data-label">在线链接数:</span>
              <span class="data-value">{{ episodeDetail.online_urls?.length || 0 }} 个</span>
            </div>
          </div>
        </div>
      </div>
...
const episodeNumber = computed(() => {
  const num = route.params.episodeNumber || route.query.episodeNumber || episodeId.value
...
const fetchEpisodeDetail = async () => {
  ...
  try {
    // 1. 首先获取动漫详情
    if (!animeId.value) {
      throw new Error('缺少动漫ID参数')
    }

    const response = await getAnimeDetail(animeId.value)

    if (response.code !== 0) {
      throw new Error(response.message || '获取动漫详情失败')
    }

    animeDetail.value = response.data

    // 2. 从动漫详情中查找特定集数的信息
    const episodes = response.data?.meta?.episodes_detail || []
    const targetEpisode = episodes.find(ep => ep.number === episodeNumber.value)

    if (!targetEpisode) {
      // 如果没有找到特定集数，使用第一个集数或创建默认数据
      episodeDetail.value = {
        id: episodeId.value,
        title: `第 ${episodeNumber.value} 集`,
        subtitle: '',
        description: '暂无本集描述信息',
        air_date: null,
        duration: null,
        view_count: 0,
        thumbnail: null,
      ...
      // 3. 映射接口数据到组件需要的数据结构
      episodeDetail.value = {
        id: targetEpisode.number,
        title: targetEpisode.title || `第 ${targetEpisode.number} 集`,
        subtitle: targetEpisode.title_cn || '',
        description: targetEpisode.description || '暂无本集描述信息',
        air_date: targetEpisode.release_date,
        duration: null, // 接口中没有时长字段
        view_count: 0,  // 接口中没有浏览数字段
        thumbnail: null, // 接口中没有缩略图字段
        online_urls: targetEpisode.online_urls || []
      }
    }
