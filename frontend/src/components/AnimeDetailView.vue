        ...
        <div class="anime-header">
          ...
          <div class="info-section">
            ...
            <div class="actions">
              <!-- 替换原来的收藏按钮 -->              
              <div class="actions">
                <div class="watch-status-buttons" :class="{ 'has-active': hasActiveStatus }">
                  <button 
                    class="status-btn want"
                    :class="{ active: watchStatus === 'want' }"
                    @click="updateWatchStatus('want')"
                    :disabled="hasActiveStatus && watchStatus !== 'want'"
                    :aria-disabled="hasActiveStatus && watchStatus !== 'want'"
                  >
                    <i class="fas fa-bookmark"></i>
                    想看
                  </button>
                  <button 
                    class="status-btn watching"
                    :class="{ active: watchStatus === 'watching' }"
                    @click="updateWatchStatus('watching')"
                    :disabled="hasActiveStatus && watchStatus !== 'watching'"
                    :aria-disabled="hasActiveStatus && watchStatus !== 'watching'"
                  >
                  ...
                  <button 
                    ...
                    :class="{ active: watchStatus === 'completed' }"
                    @click="updateWatchStatus('completed')"
                    :disabled="hasActiveStatus && watchStatus !== 'completed'"
                    :aria-disabled="hasActiveStatus && watchStatus !== 'completed'"
                  >
...
// 必须定义这个函数，否则代码运行到这里会报错停止
const copyToClipboard = async () => {
  try {
    await navigator.clipboard.writeText(window.location.href);
    alert('链接已复制！'); // 暂时用 alert 测试，确保能看到效果
  } catch (err) {
...
const fetchWatchStatus = async () => {
  try {
    ...
    if (response.data.code === 200 && response.data.data) {
      ...
      watchStatus.value = STATUS_MAP[backendStatus] || ''
    }
  } catch (error) {
    console.error('获取观看状态失败:', error)
    // 如果是404，说明还没有设置过状态，这是正常的
    if (error.response?.status === 404) {
      // 404 是正常的，说明用户还没有设置过状态
      watchStatus.value = '' // 清空状态
    } else {
      console.error('获取观看状态异常:', error.response?.data)
    }
  }
}
...
const updateWatchStatus = async (status) => {
  try {
    ...
    // 如果点击的是当前激活的状态，则取消状态
    if (watchStatus.value === status) {
      // 调用删除接口取消状态
      response = await apiClient.delete('/watch-status/', {
        params: {
          anime_id: animeId.value
        }
      })
      ...
      if (response.data.code === 200) {
        watchStatus.value = ''
        showSuccessToast('已取消观看状态')
        
        // 触发全局事件，通知其他组件动态已更新
        window.dispatchEvent(new CustomEvent('watch-status-updated', { 
          detail: { 
            animeId: animeId.value,
            status: '',
            animeTitle: animeDetail.value?.basic?.title,
            action: '取消追番'
          } 
        }))
      } else {
        throw new Error(response.data.message || '取消状态失败')
      }
    } else {
      // 若已有其他激活状态，必须先取消后再设置
      if (watchStatus.value && watchStatus.value !== status) {
        showErrorToast('已设置观看状态，请先点击当前状态以取消')
        return
      }
      // 正常设置新状态
      response = await apiClient.post('/watch-status/', {
      ...
      if (response.data.code === 200) {
        ...
        showSuccessToast(`已标记为${statusText[status]}`)
    ...
    const msg = error.response?.data?.message || '更新状态失败，请稍后重试'
    showErrorToast(msg)
  }
}
...
.status-btn.watching.active {
  background: linear-gradient(135deg, #ff6b9d, #ff8eb4);
}

.status-btn.completed.active {
  background: linear-gradient(135deg, #f79ee8, #fcb5d1);
}

/* 只有在有激活状态时，其他按钮才变暗 */
.watch-status-buttons.has-active .status-btn:not(.active) {
  opacity: 0.6;
  cursor: not-allowed;
  transform: none;
}

.watch-status-buttons.has-active .status-btn:not(.active):hover {
  background: transparent;
  transform: none;
  opacity: 0.6;
}
