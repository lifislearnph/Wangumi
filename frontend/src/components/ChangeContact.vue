  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/verify_code/`, {
      ...
      body: JSON.stringify({
        email: currentEmail.value,
        code: currentEmailCode.value,
        purpose: 'register',  // 根据API文档，固定为register
        consume: true         // 根据API文档，固定为true
      })
    })
    
    console.log('验证邮箱响应状态:', response.status)
    
    let result
    try {
      result = await response.json()
      console.log('验证邮箱响应数据:', result)
    } catch (e) {
    ...
    if (!response.ok) {
      const errorMsg = result.error || result.detail || result.message || '验证失败'
      throw new Error(errorMsg)
    }
    
    // 验证成功，进入下一步
    console.log('邮箱验证成功，进入第二步')
    step.value = 2
    
  } catch (error) {
    errorMessage.value = error.message || '验证失败，请检查邮箱和验证码'
    console.error('Error:', error)
  } finally {
    loading.value = false
  }
